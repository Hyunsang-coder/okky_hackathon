...
## ADR-022: 결과 다운로드 — Blob 기반 클라이언트 사이드 파일 생성

### 상태
승인됨

### 맥락
분석 완료 후 결과를 외부에 공유하거나 로컬에 보관할 수 있는 수단이 없었다. FR-7(URL 공유)은 서버 사이드 저장소가 필요하여 미구현 상태이므로, 서버 의존 없이 즉시 사용 가능한 파일 다운로드 방식을 먼저 제공한다.

### 결정
`lib/export.ts`에 클라이언트 사이드 다운로드 유틸리티를 신설한다:

**다운로드 방식:**
- `Blob` → `URL.createObjectURL` → 동적 `<a>` 생성·클릭 → `revokeObjectURL`
- 서버 API 호출 없이 브라우저에서 즉시 파일 생성

**두 가지 포맷:**
1. `downloadJSON(idea, report, meta, searchContext)` — 구조화된 JSON (판정, 확신도, 섹션 배열, 검색 컨텍스트 포함)
2. `downloadMarkdown(idea, report)` — 아이디어·일시 헤더 + 원본 리포트 마크다운

**파일명 규칙:** `vibcheck-결과-YYYYMMDD.json` / `.md`

**UI 배치:**
- `app/check/page.tsx`의 기존 액션 버튼 영역에 "JSON 다운로드", "MD 다운로드" 버튼 2개 추가
- 기존 "새 아이디어 검증하기" 링크와 동일한 스타일, `flex-wrap`으로 모바일 대응

### 근거
- 서버 저장소·DB 없이 구현 가능하여 인프라 추가 비용 제로
- `ReportMeta` 타입(ADR-012)을 그대로 활용하여 JSON 구조가 내부 데이터 모델과 일치
- Blob API는 모든 모던 브라우저에서 지원되며 외부 의존성 불필요
- FR-7(URL 공유)과 상호 보완적 — URL 공유는 "온라인 공유", 파일 다운로드는 "오프라인 보관·전달"

---

## ADR-023: 서버 인메모리 TTL 캐시 + 클라이언트 중복 감지

### 상태
승인됨

### 맥락
매 분석 요청마다 모든 외부 API를 새로 호출한다 (GitHub 6+쿼리, Tavily 4쿼리, Claude Haiku 1회). 동일/유사 아이디어 재검증 시에도 동일한 호출이 반복되어 토큰과 API 쿼터가 낭비된다.

### 결정

**서버 인메모리 TTL 캐시 (`lib/cache.ts`):**
- `TTLCache<T>` 제네릭 클래스: `Map` 기반, TTL 만료 자동 정리, maxSize 초과 시 LRU 삭제
- 캐시 키: `normalizeIdea()` → `djb2` 해시로 생성, 공백/대소문자/구두점 정규화
- 파이프라인 단계별 캐시 인스턴스:

| 캐시 | TTL | maxSize | 근거 |
|------|-----|---------|------|
| `keywordCache` | 24시간 | 200 | 같은 아이디어 → 같은 키워드, 모델 변경 없음 |
| `githubCache` | 1시간 | 100 | GitHub 데이터는 분 단위로 변하지 않음 |
| `tavilyCache` | 30분 | 100 | 웹 검색은 더 자주 변함 (특히 뉴스/트렌드) |

- 리포트 생성(Phase 4)은 캐시하지 않음 — 같은 아이디어라도 새 관점의 리포트를 제공

**캐시 적용 위치 (`app/api/analyze/route.ts`):**
- 파이프라인 함수(extractKeywords, searchGitHub, searchTavily) 자체는 순수하게 유지
- 캐시 조회/저장은 호출부(route.ts)에서 래핑
- 캐시 HIT 시 progress 이벤트에 "(캐시)" 표시, HIT이어도 progress 이벤트는 정상 발송

**클라이언트 중복 감지 (`lib/history.ts` + `app/page.tsx`):**
- `findMatchingHistory(idea)`: 로컬스토리지 히스토리에서 정규화된 아이디어 매칭
- 매칭 시 다이얼로그 표시: "이전 결과 보기" / "새로 분석하기" 선택

### 근거
- 서버 인메모리 캐시는 인프라(Redis 등) 추가 없이 즉시 적용 가능
- Vercel Serverless 환경에서 cold start 시 캐시가 비어있지만, warm instance에서는 효과적
- 파이프라인 함수를 수정하지 않으므로 기존 로직에 영향 없음
- 클라이언트 중복 감지는 서버 호출 자체를 방지하여 가장 확실한 비용 절감
- djb2 해시는 crypto 모듈 없이 동작하여 클라이언트/서버 공용 사용 가능
