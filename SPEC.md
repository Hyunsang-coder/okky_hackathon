...
- **서버 인메모리 TTL 캐시**: 키워드 추출(24시간), GitHub 검색(1시간), Tavily 검색(30분) 결과를 캐시하여 동일/유사 아이디어 재검증 시 외부 API 호출을 제거한다
- **클라이언트 중복 감지**: 홈에서 이전에 검증한 동일 아이디어 입력 시 다이얼로그로 안내하여 불필요한 재분석을 방지한다
